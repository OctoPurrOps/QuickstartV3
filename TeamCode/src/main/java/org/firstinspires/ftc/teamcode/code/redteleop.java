...
public class redteleop extends OpMode {
    ...
    private static final Pose START_POSE = new Pose(88, 16, Math.toRadians(90));
    ...
    // NEW: dpad‑up + lock-at-zero state
    ...
    public void init() {
        ...
        // Reset encoder at init so current physical angle is 0 reference
    ...
    public void loop() {
        ...
        if (gamepad1.dpad_up && !dpadUpLatch) {
            ...
            if (lockAimAtZero) {
                // Define current angle as 0 and hold it
                aimMotor.setMode(DcMotor.RunMode.STOP_AND_RESET_ENCODER);
                aimMotor.setTargetPosition(0);
                aimMotor.setMode(DcMotor.RunMode.RUN_TO_POSITION);
                aimMotor.setPower(AIM_HOLD_POWER);
            } else {
                // Release lock, go back to encoder-based open control
