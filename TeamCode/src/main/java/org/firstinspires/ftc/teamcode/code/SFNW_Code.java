...
@TeleOp
public class SFNW_Code extends OpMode {
    ...
    public void init() {
        ...
        // Init intake (always runs)

        intake.setPower(1);  // Continuous intake power (adjust as needed)
        intake2.setPower(1);  // Continuous intake power (adjust as needed)
    ...
    public void loop() {
        ...
        if (gamepad1.xWasPressed()) anglePos = Math.min(1.0, anglePos + 0.01);
        if (gamepad1.bWasPressed()) anglePos = Math.max(0.0, anglePos - 0.01);
        angleServo.setPosition(anglePos);
